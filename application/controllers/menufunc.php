...
class Menufunc_Controller extends Controller
{
	function makeListItemsFromNodes($nodes)
	{
		$menutree = "";
		$clevel=1;
		...
		foreach($nodes as $i => $row)
		{
			...
			if($row->node_or_leaf == "L")
			{
				...
				if($row->label_enquiry != "")
				{
					...
					if ($pos_e > 0){$row->label_enquiry = sprintf('<img src="%smedia/img/menu/%s" align=absbottom>',$baseurl,str_replace("%IMG%","",$row->label_enquiry));}
				}
				if($row->label_input != "")
				{
					$pos_i=strpos(trim($row->label_input),"%IMG%");
					if ($pos_i > 0){$row->label_input = sprintf('<img src="%smedia/img/menu/%s" align=absbottom>',$baseurl,str_replace("%IMG%","",$row->label_input));}
	...
	function makeSecurityProfile($nodes)
	{
		$menutree = "";
	...
	function makeMenuSelectionList($nodes,$sparr='')
	{
		$menutree = "";
		$clevel=1;
		...
		foreach($nodes as $i => $row)
		{
			...
			if($row->controls_input !="")
			{
				...
				//$iperm_checkboxs = '$iperm_checkboxs.'</span>';
			}
			...
			if($row->controls_enquiry !="")
			{
				...
				//$eperm_checkboxs = '<span class="ce""> '.$eperm_checkboxs.'</span>';
			}
