...
class Inventupdtype_Controller extends Site_Controller
{
	...
	function input_validation()
	{
		...
		$validation->add_rules('update_type_id','required', 'length[3,50]', 'standard_text');
		$validation->add_rules('description','required', 'length[1,255]', 'standard_text');
		$validation->add_rules('stock_movement','required', 'length[3,20]', 'standard_text');

		$validation->add_callbacks('update_type_id', array($this, '_duplicate_altid'));
    ...
    {
		$id	 = $_POST['id'];
		$unique_id = $_POST['update_type_id'];
