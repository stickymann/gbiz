...
define("DELIMITER","{##}");
class Ajaxtodb_Controller extends Controller
{
	
	public function __construct()
    {
		$this->sitedb = new Site_Model;
		$this->enqdb = new Enqdb_Model;
	}
...
//print $querystr."<br>";				
				...
				$this->printSideInfoResult($result);
			break;
...
//print_r($_REQUEST);	print "<hr>";				

				if($enqtype == 'default')
				{
					$fields = join(',',$this->sitedb->getFormFields($controller));
					...
					$label = array();
					$form = $this->sitedb->getFormFields($controller,$label);
					Site_Controller::mergeFormWithAuditFields($form,$label);

				}
				elseif($enqtype == 'custom')
				{
					$this->enqdb->getEnqFormFields($controller,$form,$label,$filterfields);
					$fields = join(',',$form);
				...
				if($export)
				{
					$this->printCSVId($controller,$result,$tabletype,$label,$fn,$user,$enqtype);
				}
				else
				{
					$this->printEnquiryResult($controller,$result,$tabletype,$label,$fn);
				}
			//END ENQUIRY//////////////////////////////////////////////////////
			break;
...
//print_r($_REQUEST);
				...
				if($rochk){$this->printFilterForm($controller,$user,$enqtype,$loadfixedvals,$rochk);}
				else {$this->printFilterForm($controller,$user,$enqtype,$loadfixedvals);}
			break;

			case 'enqctrl':
				$controller = $_REQUEST['controller'];
				$idname		= $_REQUEST['user'];
				$this->printEnquiryControls($controller,$idname);
			break;
	...
	function printSideInfoResult($result)
	{
	...
	function printPopOutResult($result)
	{
		$idfield = $_REQUEST['idfield'];
...
_TEXT_;
	...
	function printEnquiryResult($controller,$result,$tabletype,$label,$fn)
	{
		...
		if($tabletype == "df") 
		{
			$arr = $this->enqdb->getEnquiryParams($controller);
			$arr = (array) $arr[$controller];
			$altidfld = $arr['idfield'];
		} 
		else 
		{
			$arr = $this->sitedb->getControllerParams($controller);
			...
			foreach ($linerec as $key => $value)
			{
				...
				if($key == $idfield)
				{
					if($tabletype=="hs")
					{
						...
						$data .= '<td>'.html::anchor($controller.'/index/'.$value_no,$value,array('target'=>'input'));
					}
					...
					{
						$data .= '<td>'.html::anchor($controller.'/index/'.$value,$value,array('target'=>'input'));
					}
				}
...
_TEXT_;
	...
	{
		$csv_id = $this->createCSV($CSV,$controller,$result,$tabletype,$label,$fn);
		$csv = new Csv_Controller();
		$csv->InsertIntoCSVTable($csv_id,$CSV,$controller,$idname,$type);
		//$RESULT = str_replace("\r\n","<br>", $CSV);
		//$csv_id = "12344675475";
		print $csv_id.DELIMITER;
	}

	function createCSV(&$CSV,$controller,$result,$tabletype,$label,$fn)
	{
		$RESULT = '';
		$firstpass = true;
		$idfield = 'id';
	...
	function printFilterForm($controller,$user,$enqtype,$loadfixedvals,$rochk=false)
	{
		if($enqtype == 'default')
		{
			$label = array();
			$form = $this->sitedb->getFormFields($controller,$label);
			Site_Controller::mergeFormWithAuditFields($form,$label);
			//$enqlookup = '<input type="hidden" id="enqtype" name="enqtype" value="default">';
		}
		elseif($enqtype == 'custom')
		{
			$this->enqdb->getEnqFormFields($controller,$form,$label,$filterfields);
			//$enqlookup = '<input type="hidden" id="enqtype" name="enqtype" value="custom">';
		}
		
		if($loadfixedvals)
		{
			$count = 0;
			if($this->enqdb->getFixedSelectionParams($controller,$fldname,$operand,$inputvalue,$attr))
			{
...
*/
		...
		{	
			$inval = "";  $attrval = "";
			$SELECT = form::dropdown(array('id'=>'%ID%','name'=>'%ID%','class'=>'ff'),$selection,'EQ');
			...
			if($loadfixedvals)
			{
				...
				if($attrval=="readonly" && array_key_exists($key,$operand))
				{
					...
					$SELECT = "\n".form::dropdown(array('id'=>'%ID%','name'=>'%ID%','class'=>'ff'),$tmpselect,$operand[$key])."\n";
				}
				else if(array_key_exists($key,$operand))
				{
					$SELECT = "\n".form::dropdown(array('id'=>'%ID%','name'=>'%ID%','class'=>'ff'),$selection,$operand[$key])."\n";
				}
				
			}
		
			if($rochk){$SELECT = "\n".form::dropdown(array('id'=>'%ID%','name'=>'%ID%','class'=>'ff','onChange'=>'window.fixedselection.setFS();'),$selection,'EQ')."\n";}
	...
	function printEnquiryControls($controller,$idname)
	{
		$enqradios = new Sitecontrol_Controller($controller,$idname);
		$html = $enqradios->getEnqFormControls();
		print $html;
	}
	...
	function printOrderStatusForm($fldval)
	{
		...
		if($result = $this->sitedb->executeSelectQuery($querystr))
		{
...
_HTML_;
	...
	function getPeriodString($param)
	{
